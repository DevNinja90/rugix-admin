          ...
          <label className="block">
            ...
            <span className="flex min-h-24 cursor-pointer items-center justify-between gap-4 rounded-lg border border-dashed border-frame bg-elevation-0 px-4 py-3 transition hover:border-primary hover:bg-primary-muted">
              <span className="min-w-0">
                <span className={classes("block text-sm font-medium text-foreground", file && "font-mono")}>{file?.name ?? fileLabel}</span>
                <span className={classes("mt-1 block truncate text-xs text-foreground-muted", file && "font-mono")}>{file ? formatBytes(file.size) : "No file selected"}</span>
              </span>
